...
class WechatMiniProgramBindController extends AuthBaseController
{
    ...
    public function main()
    {
        try {
            ...
            app('errorLog')->info('requestId：' . $this->requestId . '-' . '小程序绑定参数获取接口异常-WechatMiniProgramLoginController： ' . $e->getMessage());
    ...
    //                $this->bound->bindVoid($sessionToken, $wechatUser);
            ...
            app('errorLog')->info('requestId：' . $this->requestId . '-' . '小程序绑定接口异常-WechatMiniProgramBindController： ' . $e->getMessage());
            $this->db->rollBack();
