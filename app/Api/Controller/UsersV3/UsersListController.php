...
class UsersListController extends DzqController
{
    ...
    public function main()
    {
        ...
        if(empty($column)){
            ...
            $likeL = User::query()->where($column,'like',$value.'%');

            $likeR = User::query()->where($column,'like','%'.$value);

            $likeM = User::query()->where($column,'like','%'.$value.'%');
            $query->union($likeL->getQuery())->union($likeR->getQuery())->union($likeM->getQuery());


            dd($query->get()->pluck('username'));
        ...
        if(!empty($filter['username'])) {
            $query->where('users.username', 'like', '%' . $filter['username'] . '%');
        }
